...
function Inputs({name,type,setState}:InputsProps) {
  return (
      <div className="mb-6 flex w-1/2 pr-4">
        ...
        <input className=" flex-grow h-8 py-2 px-4 focus:outline-none" name={name} type={type} placeholder={`Enter your ${name}`} onChange={(e)=>type ==='number' ? setState(parseInt(e.target.value)) : setState(e.target.value)}/>
      </div>
  );
}
